...
class _plotDepth(pg.PlotWidget):
    def __init__(self, *args, **kwargs):
        ...
        self.depth_plot.getViewBox().invertY(True)
...
class IMUGraph(QtWidgets.QWidget):
    ...
    def update(self, arg):
        if len(self.plot.time) > 500:
            ...
            self.plotDepth.time = self.plot.time[1:]
            ...
            self.plotDepth.time.append(times)
            self.plot.roll_data.append(arg[0])
            self.plot.pitch_data.append(arg[1])
            self.plot.yaw_data.append(arg[2])
            self.plotDepth.depth_data.append(arg[3])
        ...
        self.plotDepth.enableAutoRange()
        ...
        self.plotDepth.depth_plot.setData(self.plot.time,self.plotDepth.depth_data)
